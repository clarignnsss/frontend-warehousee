...
const ProductAttribute = () => {
  ...
  const [IdAttrValue, setIdAttrValue] = useState();
  ...
  const handleSubmit = (evt) => {
    ...
    response
      ...
      .then((result) => {
        ...
        fetch(`http://localhost:4000/api/productattr/${id}`)
          .then((res) => {
            return res.json();
          })
          .then((result) => {
            console.log(result);
            setProductattr([...productattr, ...result]);
          });
        setAlert(true);
        if (result.protocol41) {
          setAlertColor("success");
          setAlertMsg("Product Attribute data successfully submitted !");
        } else {
